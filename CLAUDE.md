...
- `Makefile` — `make cli` downloads the Looker CLI to `./looker-cli` (gitignored, SHA-256 verified; pin with `LOOKER_CLI_VERSION`). `make login` / `make check` wrap `./lk`. `make medium [ARTICLE=docs/x.md]` runs `scripts/medium.py`.
- `scripts/medium.py` — derives the Medium variant of a dev.to article: strips frontmatter, promotes the title to an H1, rewrites tables as bullet lists (Medium renders neither), and pandocs the result to `<name>-medium.html`. Tables inside code fences are left alone. The `-medium.md` files are **generated** — edit the dev.to source and re-run, never hand-edit them.
