...
![Looker + MCP — Configuring the Codex CLI](https://raw.githubusercontent.com/xbill9/looker-mcp-codex/main/docs/images/cover-codex.jpg)
...
Two of those lines deserve more than a bullet.
...
- Cause: Project not trusted, so `.codex/config.toml` never loaded
- Fix: Restart `codex` in the repo root and approve the trust prompt

**Server fails immediately**
- Cause: No `.env` and no exported `LOOKER_*`
- Fix: `source set_env.sh` — the launcher prints exactly this

**Tools start failing with 401**
- Cause: `LOOKER_MCP_TOKEN` expired (Looker access tokens last one hour)
- Fix: `export LOOKER_MCP_TOKEN=$(./lk token)` and restart Codex

**`run_dashboard` times out**
- Cause: Ten tiles = ten warehouse queries
- Fix: Raise `tool_timeout_sec`, or run tiles individually

**All `health_*` return Access Denied**
- Cause: API3 role lacks `see_system_activity`
- Fix: Admin → Roles, add the permission

**Filter returns nothing**
- Cause: Value was quoted
- Fix: Pass values bare — `first_touch`, not `"first_touch"`

**Tools run without asking**
- Cause: Approval mode not applied
- Fix: `codex mcp get looker-managed` — if the key isn't echoed back it was misspelled and silently dropped. Also check you didn't pick "don't ask again" earlier in the session
